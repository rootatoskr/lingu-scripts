...
async function dragTo(el, from, to, steps = 12) {
    const opts = { bubbles: true, cancelable: true, composed: true, pointerId: 1, pointerType: 'mouse', isPrimary: true, button: 0 }
    el.dispatchEvent(new PointerEvent('pointerdown', { ...opts, clientX: from.x, clientY: from.y, buttons: 1 }))
    el.dispatchEvent(new MouseEvent('mousedown', { ...opts, clientX: from.x, clientY: from.y, buttons: 1 }))
    ...
    window.dispatchEvent(new PointerEvent('pointerup', { ...opts, clientX: to.x, clientY: to.y, buttons: 0 }))
    window.dispatchEvent(new MouseEvent('mouseup', { ...opts, clientX: to.x, clientY: to.y, buttons: 0 }))
}
...
function getWordPhrase(el) {
    ...
    let depth = 0
    while (fiber && depth < 8) {
        if (fiber.memoizedProps && fiber.memoizedProps.word && fiber.memoizedProps.word.phrase) return fiber.memoizedProps.word.phrase
        fiber = fiber.return
        depth++
    }
    return null
...
async function runOneRound() {
    ...
    const pairs = wordDivs.map(w => {
        ...
        const audio = audioDivs.find(a => a.textContent.trim() === translation)
        return { translation, wordEl: w, audioEl: audio }
    })

    let done = 0
    for (const p of pairs) {
        if (!p.audioEl) { console.log('no match for', p.translation); continue }
...
async function runMatch(maxRounds = 30) {
    ...
    if (!checkTaskType(data.type, 'Tasks::WordGames')) return
    ...
    while (round < maxRounds) {
        if (!await waitFor(() => {
            const words = [...document.querySelectorAll('[data-type="word"]')]
        ...
        prevSig = [...document.querySelectorAll('[data-type="word"]')].map(w => w.dataset.projectionId).join(',')
        round++
        console.log('round', round, 'matched', done, 'pairs')
    }
    console.log('total', Math.round(performance.now() - t0), 'ms', 'rounds =', round)
}
