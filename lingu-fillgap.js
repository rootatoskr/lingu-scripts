...
function dropdownSegments(groupCount) {
    ...
    const starts = []
    btns.forEach((b, i) => { if (norm(b.textContent) === '•••') starts.push(i) })
...
async function run(delay = 300) {
    ...
    if (!checkTaskTypeMulti(data.type, ['Tasks::FillGap', 'Tasks::InlineDropdown'])) return
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (!await waitStable(() => passedCount() === want && allAnswersOnScreen(groups) && dropdownSegments(groups.length) !== null)) {
            console.log(i + 1, 'not ready, passed=', passedCount(), 'want', want, buttonTexts().join('|'))
            break
        }

        const segs = dropdownSegments(groups.length)
        if (!segs) { console.log(i + 1, 'no segments', buttonTexts().join('|')); break }
        ...
        for (let k = 0; k < words.length; k++) {
            ...
            if (!btn) { console.log(i + 1, 'no button for', words[k], 'in segment', k, '::', seg.map(b => norm(b.textContent)).join('|')); failed = true; break }
            targets.push(btn)
        }
        if (failed) break
        ...
        if (!await waitNext(want)) { console.log(i + 1, 'not accepted, passed=', passedCount()); break }
    }
