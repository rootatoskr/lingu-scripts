...
function wordButtons() {
    return [...document.querySelectorAll('button[data-value]')].filter(b => !b.disabled && !b.classList.contains('hide'))
}
...
async function run(delay = 250) {
    ...
    if (!checkTaskType(data.type, 'Tasks::ArrangeWords')) return
    ...
    console.log('base passed =', base, 'dashes =', dashes, 'items =', items.length)
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (!await waitStable(() => passedCount() === want && wordsOnScreen(allWords))) {
