...
const LINGU_TYPE_TO_SCRIPT = {
    ...
    "Tasks::FillInBlanks": "lingu-selecttext.js",
};
...
function findOptionLabelForQuestion(questionText, answerText, occurrenceIndex) {
    ...
    const matches = headings.filter(
        (h) => norm(h.textContent) === norm(questionText),
    );
    ...
    const inRange = allLabels.filter((label) => {
        const afterHeading =
            (headingPos(label) & Node.DOCUMENT_POSITION_FOLLOWING) !== 0;
        if (!afterHeading) return false;
        if (nextHeading) {
            const afterNext =
                (nextHeading.compareDocumentPosition(label) &
    ...
    return (
        inRange.find((label) => norm(label.textContent) === norm(answerText)) ||
        null
    );
}
...
async function runSelectText(items, delay) {
    ...
    for (const item of items) {
        ...
        if (!correct) {
            console.log(item.question, "-> no correct option in data");
            continue;
        }
        ...
        const el = findOptionLabelForQuestion(
            item.question,
            correct.answer,
            occurrenceIndex,
        );
...
// ---------- FillInTable / FillInBlanks (спільна логіка вводу тексту) ----------
...
async function runFillInTable(items) {
    ...
    const inputs = findTableInputs();

    if (inputs.length !== sorted.length) {
        console.log(
            `Невідповідність: items=${sorted.length}, знайдених полів=${inputs.length} -> не заповнюю, перевір DOM`,
        );
        return;
    }
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        if (norm(input.value).length > 0) {
            console.log(item.id, "already filled:", input.value);
            continue;
        }

        typeText(input, answer);
        console.log(item.id, "->", answer);
        filled++;
    }
    console.log("done, filled", filled, "of", sorted.length);
}
...
async function runFillInBlanks(items) {
    const sorted = [...items].sort((a, b) => a.position - b.position);
    const inputs = findBlankInputs();
    ...
    for (let i = 0; i < sorted.length; i++) {
        const item = sorted[i];
        const input = inputs[i];
        const answer = item.solution[0][0];

        if (norm(input.value).length > 0) {
            console.log(item.question, "already filled:", input.value);
            continue;
        }
...
function findTrueFalseButton(statement, correct) {
    ...
    for (const label of labels) {
        ...
        const btn = container.querySelector(
            `button[role="checkbox"][aria-label="${wanted}"]`,
        );
        if (btn) return btn;
    }
    return null;
...
async function runTrueFalse(items, delay) {
    ...
    for (const item of items) {
        ...
        if (!btn) {
            console.log(item.statement, "-> button not found");
            continue;
        }
        realClick(btn);
...
async function run(delay = 300) {
    ...
    } else if (data.type === "Tasks::FillInBlanks") {
        await runFillInBlanks(items);
    } else {
